...
router.post("/new/:brewery_id", middleware.isAuthorized, async (req, res) => {
  try {
    const review = await db.query(
      "SELECT * FROM reviews WHERE user_id = $1 AND brewery_id = $2",
      [req.user.id, req.params.brewery_id]
    );
    ...
    const newReview = await db.query(
      ...
      [req.user.id, req.params.brewery_id, req.body.body, req.body.rating]
    );
...
//UPDATE REVIEW ROUTE
router.put("/update/:review_id", middleware.isAuthorized, async (req, res) => {
  try {
    const updateReview = await db.query(
      "UPDATE reviews SET body = $1, rating = $2 WHERE review_id = $3 AND user_id = $4 RETURNING *",
      [req.body.body, req.body.rating, req.params.review_id, req.user.id]
    );
  ...
  async (req, res) => {
    try {
      const deleteReview = await db.query(
        ...
        [req.params.review_id, req.user.id]
      );
